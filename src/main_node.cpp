...
bool setup(rovi_srvs::DigitalFilter::Request &req,rovi_srvs::DigitalFilter::Response &res){
	ROS_INFO("mainx::setup called: %d",req.data.size());
	return true;
}
bool mainx(rovi_srvs::GenPC::Request &req,rovi_srvs::GenPC::Response &res){
	ROS_INFO("mainx called: %d %d",req.imgL.size(),req.imgR.size());
...
bool trypc(std_srvs::Trigger::Request &req,std_srvs::Trigger::Response &res){
	rovi_srvs::GenPC srv;
	return mainx(srv.request,srv.response);
}
int main(int argc, char **argv){
	ros::init(argc,argv,"main_node");
	ros::NodeHandle n;
	nh=&n;
	ros::ServiceServer svc0=n.advertiseService("mainx/setup",setup);
	ros::ServiceServer svc1=n.advertiseService("mainx/do",mainx);
	ros::ServiceServer svc2=n.advertiseService("mainx/try",trypc);
	ros::Publisher p1=n.advertise<sensor_msgs::PointCloud>("mainx/pcl",1);
	pub1=&p1;
	ros::Publisher p2=n.advertise<sensor_msgs::PointCloud2>("mainx/pcl2",1);
	pub2=&p2;
	ros::spin();
	return 0;
}
